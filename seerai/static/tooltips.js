...
(function () {
  ...
  function efficiencyContent(vals) {
    ...
    let verdict;
    if (r === null) {
      verdict = '<div class="tt-verdict tt-neutral">No subscription data available to compute efficiency.</div>';
    } else if (r < 0.3) {
      const saving = Math.round((1 - r) * 100);
      verdict = `<div class="tt-verdict tt-warn">At <strong>${r.toFixed(2)}x</strong>, actual API usage is far below subscription cost. Switching to per-use API billing could save ~${saving}% on AI spend.</div>`;
    } else if (r < 0.8) {
      verdict = `<div class="tt-verdict tt-warn">At <strong>${r.toFixed(2)}x</strong>, usage is moderate relative to subscription cost. Per-use billing would be cheaper, though subscriptions offer predictable budgeting.</div>`;
    } else if (r <= 1.3) {
      verdict = `<div class="tt-verdict tt-good">At <strong>${r.toFixed(2)}x</strong>, you're in the healthy zone &mdash; subscription cost closely matches actual usage value.</div>`;
    } else {
      verdict = `<div class="tt-verdict tt-good">At <strong>${r.toFixed(2)}x</strong>, the subscription is saving money &mdash; equivalent API billing would cost ${r.toFixed(1)}x more.</div>`;
    }
  ...
  function valueContent(vals) {
    ...
    if (sub > 0 && v > 0) {
      const ratio = v / sub;
      if (ratio > 5) {
        context = `<div class="tt-verdict tt-good">Estimated time savings of <strong>${fmt$(v)}</strong> against <strong>${fmt$(sub)}/mo</strong> in subscriptions &mdash; AI tools appear to be generating strong value.</div>`;
      } else if (ratio > 1) {
        context = `<div class="tt-verdict tt-neutral">Estimated time savings of <strong>${fmt$(v)}</strong> against <strong>${fmt$(sub)}/mo</strong> in subscriptions &mdash; positive return, with room to grow through higher-utility usage.</div>`;
      } else {
        context = `<div class="tt-verdict tt-warn">Estimated savings of <strong>${fmt$(v)}</strong> don't yet exceed the <strong>${fmt$(sub)}/mo</strong> subscription cost. Check if sessions are being classified correctly and if users need onboarding support.</div>`;
      }
    }
  ...
  function roiContent(vals) {
    ...
    let verdict;
    if (r === null) {
      verdict = '<div class="tt-verdict tt-neutral">No subscription cost to compute ROI against.</div>';
    } else if (r >= 10) {
      verdict = `<div class="tt-verdict tt-good">At <strong>${r.toFixed(1)}x</strong>, AI tools are generating exceptional value relative to their cost.</div>`;
    } else if (r >= 2) {
      verdict = `<div class="tt-verdict tt-good">At <strong>${r.toFixed(1)}x</strong>, solid return &mdash; AI tools are clearly paying for themselves.</div>`;
    } else if (r >= 1) {
      verdict = `<div class="tt-verdict tt-neutral">At <strong>${r.toFixed(1)}x</strong>, value slightly exceeds cost. Look for opportunities to increase useful-session ratio.</div>`;
    } else {
      verdict = `<div class="tt-verdict tt-warn">At <strong>${r.toFixed(1)}x</strong>, subscription cost exceeds estimated value. Review whether utility classifications are accurate and whether some users need more onboarding.</div>`;
    }
