...
Each locale picks an industry that's typical for the country:
  ...
  ru / RU → Banking & Finance (Волга Банк / Невский Капитал)
...
class LocaleConfig:
    ...
    lang: str          # "en", "de", "it", "ru"
    country: str       # "US", "DE", "IT", "RU"
    industry: str      # human-readable
    currency: str      # "USD", "EUR", "EUR", "RUB"
...
# ────────────────────────────────────────────────────────────────────────
# RU / Russia / Banking & Finance
# ────────────────────────────────────────────────────────────────────────

RU_LOCALE = LocaleConfig(
    lang="ru",
    country="RU",
    industry="Банковское дело и финансы",
    currency="RUB",
    org_tree={
        "volga": {
            "name": "Волга Банк",
            "children": {
                "volga-retail": {
                    ...
                    "children": {
                        "volga-retail-mortgage":  {"name": "Ипотека"},
                        "volga-retail-cards":     {"name": "Карты и платежи"},
                        "volga-retail-loans":     {"name": "Потребкредитование"},
                    },
                },
                "volga-corporate": {
                    "name": "Корпоративный бизнес",
                    "children": {
                        "volga-corporate-large": {"name": "Крупный корпбиз"},
                        "volga-corporate-sme":   {"name": "МСБ"},
                    },
                },
                "volga-sales": {"name": "Продажи"},
            },
        },
        "nevsky": {
            "name": "Невский Капитал",
            "children": {
                "nevsky-invest": {
                    "name": "Инвестиции",
                    "children": {
                        "nevsky-invest-algo": {"name": "Алготрейдинг"},
                    },
                },
                "nevsky-ops": {"name": "Операции"},
            },
        },
    },
    users={
        "volga-retail-mortgage": [
            "alexey.ivanov", "dmitry.petrov", "ekaterina.volkova",
            "maxim.sokolov", "olga.morozova",
        ],
        "volga-retail-cards":   ["nikolay.fedorov", "anna.popova", "sergey.vasiliev"],
        "volga-retail-loans":   ["irina.stepanova", "pavel.romanov"],
        "volga-corporate-large": ["boris.kuznetsov", "tatiana.lebedeva", "viktor.mikhailov"],
        "volga-corporate-sme":   ["natalia.smirnova", "roman.pavlov"],
        "volga-sales": [
            "elena.belova", "igor.tarasov", "mikhail.orlov", "yulia.kiselyova",
        ],
        "nevsky-invest-algo":    ["alexander.egorov", "ksenia.borisova", "andrey.komarov"],
        "nevsky-ops":            ["vadim.antonov", "marina.zaitseva"],
    },
    execs={"alexey.ivanov", "boris.kuznetsov", "elena.belova", "alexander.egorov"},
    hourly_rates={
        # In roubles. Russian banking IT/quant pay is well above retail front-office.
        "volga-retail-mortgage": (2000, 4000),
        "volga-retail-cards":    (2500, 5000),
        "volga-retail-loans":    (2000, 4000),
        "volga-corporate-large": (3500, 7000),
        "volga-corporate-sme":   (2500, 5000),
        "volga-sales":           (1500, 3500),
        "nevsky-invest-algo":    (4500, 9000),
        "nevsky-ops":            (1500, 3500),
    },
    display_names={
        "alexey.ivanov":     "Алексей Иванов",
        "dmitry.petrov":     "Дмитрий Петров",
        "ekaterina.volkova": "Екатерина Волкова",
        "maxim.sokolov":     "Максим Соколов",
        "olga.morozova":     "Ольга Морозова",
        "nikolay.fedorov":   "Николай Фёдоров",
        "anna.popova":       "Анна Попова",
        "sergey.vasiliev":   "Сергей Васильев",
        "irina.stepanova":   "Ирина Степанова",
        "pavel.romanov":     "Павел Романов",
        "boris.kuznetsov":   "Борис Кузнецов",
        "tatiana.lebedeva":  "Татьяна Лебедева",
        "viktor.mikhailov":  "Виктор Михайлов",
        "natalia.smirnova":  "Наталья Смирнова",
        "roman.pavlov":      "Роман Павлов",
        "elena.belova":      "Елена Белова",
        "igor.tarasov":      "Игорь Тарасов",
        "mikhail.orlov":     "Михаил Орлов",
        "yulia.kiselyova":   "Юлия Киселёва",
        "alexander.egorov":  "Александр Егоров",
        "ksenia.borisova":   "Ксения Борисова",
        "andrey.komarov":    "Андрей Комаров",
        "vadim.antonov":     "Вадим Антонов",
        "marina.zaitseva":   "Марина Зайцева",
    },
    user_messages=[
        "Как нам соответствовать Положению ЦБ 716-П по управлению операционным риском?",
        "Проверь расчёт норматива достаточности капитала Н1.0 по нашей модели.",
        "Помоги подготовить форму 0409101 и 0409102 для отчётности в ЦБ РФ.",
        "Какие ключевые изменения внесли последние поправки в 115-ФЗ?",
        "Объясни подход IRB-A к розничному ипотечному портфелю по Базель III.",
        "Как рассчитать LGD для ипотеки с учётом нашей внутренней модели восстановления?",
        "Опиши полный процесс KYC для юридических лиц — нерезидентов.",
        "Какая ключевая ставка ЦБ действует на сегодня и когда следующее заседание?",
        "Как правильно оформить уведомление в Росфинмониторинг по 115-ФЗ об подозрительной операции?",
        "Набросай структуру внутреннего регламента по обработке ПДн держателей карт в соответствии с 152-ФЗ.",
        "Составь бриф для запуска новой ипотечной программы для IT-специалистов.",
        "Объясни разницу между стандартизированным подходом и IRB в расчёте RWA.",
        "Как настроить правила антифрода для P2P-переводов через СБП?",
        "Подготовь чек-лист интеграции с ФГИС Биометрия для удалённой идентификации.",
    ],
    ai_responses=[
        "Положение ЦБ 716-П требует: (1) карту операционных рисков с привязкой к бизнес-процессам, (2) базу инцидентов с ежедневным пополнением, (3) ключевые индикаторы риска (KRI) по каждому направлению, (4) ежегодный стресс-тест по ОР, (5) отчёт в ЦБ по форме 0409106 ежеквартально. Ответственность — КУОР (Комитет по управлению операционным риском).",
        "Норматив Н1.0 = Собственные средства / (КРВ + КРС + ОР × 12.5) ≥ 8%. Проверьте три места: (а) вычеты из капитала по отложенным налоговым активам, (б) взвешивание по риску корпоративных кредитов — по умолчанию 100%, по IRB — от 20% до 150%, (в) добавок за операционный риск по базовому индикативному подходу (15% от среднего валового дохода за 3 года).",
        "Формы 0409101 (оборотная ведомость) и 0409102 (отчёт о фин. результатах) сдаются ежемесячно не позднее 4-го рабочего дня. Критичные моменты: сверка остатков по счетам второго порядка, контроль по символам отчёта 0409102, выверка расхождений с балансом до подписи. Сдача через АСПО, формат XML по схеме ЦБ.",
        "Последние поправки в 115-ФЗ (ред. 2024 г.): (1) ужесточены требования к идентификации бенефициарных владельцев для юрлиц, (2) расширен перечень подозрительных операций (включены операции с цифровыми финансовыми активами), (3) введены новые сроки отправки сообщений в Росфинмониторинг — 1 рабочий день вместо 3, (4) обязательный риск-ориентированный подход для всех клиентов.",
        "IRB-A для розничной ипотеки: оценка PD на горизонте 1 год по внутренней модели (минимум 5 лет данных, скоринг по LTV, PTI, кредитной истории), LGD с учётом залога и времени реализации (обычно 20-35%), EAD = непогашенный остаток. RWA = 12.5 × K × EAD, где K рассчитывается по формуле Базеля с корреляцией 0.15 для розницы. Валидация модели ежегодно.",
        "LGD по ипотеке в вашем контексте: LGD = 1 − RR, где RR (recovery rate) = (Выручка от реализации залога − Расходы на реализацию) / EAD. Учитывайте: средний дисконт к рыночной стоимости 25-35% при вынужденной продаже, срок реализации 9-18 месяцев (дисконтирование по ставке портфеля), судебные и нотариальные расходы ~3-5%. Минимальный LGD для ипотеки по Базелю — 10%.",
        "KYC для юрлиц-нерезидентов: (1) учредительные документы с апостилем и нотариальным переводом, (2) выписка из реестра страны регистрации не старше 3 мес., (3) подтверждение налогового резидентства (сертификат), (4) структура собственности до конечного бенефициара (≥25%), (5) проверка по спискам СДН/OFAC/ЕС и внутренним чёрным спискам, (6) анкета ПОД/ФТ, (7) обоснование экономической целесообразности операций в РФ.",
        "Ключевая ставка ЦБ на данный момент — 21% годовых (установлена 25.10.2024, действует с 28.10.2024). Следующее опорное заседание Совета директоров — обычно последняя пятница месяца; решение публикуется в 13:30 МСК. Для актуальных данных уточните на cbr.ru/hd_base/keyrate — я могу ошибаться по датам.",
        "Уведомление в Росфинмониторинг по ст. 6 и ст. 7.3 115-ФЗ оформляется в виде формализованного электронного сообщения (ФЭС) через личный кабинет на portal.fedsfm.ru. Ключевые поля: код вида операции, сумма, признак необычности (коды 11хх-22хх), данные клиента и контрагента, обоснование подозрений. Срок — 1 рабочий день с момента выявления признаков. При блокировке счёта — не позднее рабочего дня, следующего за днём отказа.",
        "Регламент обработки ПДн держателей карт по 152-ФЗ + PCI DSS: (1) согласие на обработку ПДн в договоре и при онлайн-оформлении (ст. 6 152-ФЗ), (2) локализация в РФ (ст. 18 ч. 5 — первичный сбор только в российских БД), (3) сроки хранения: активные — весь срок действия карты + 5 лет, маскирование PAN в логах, (4) доступ по ролевой модели, (5) уведомление РКН о нарушениях в течение 24 ч (ст. 21 ч. 3.1).",
    ],
    error_messages=[
        "Превышен лимит запросов. Повторите попытку через 30 секунд.",
        "Превышен размер контекста. Разбейте запрос на несколько частей.",
        "Сервис временно недоступен. Модель обновляется.",
        "Некорректный ввод: сообщение превышает максимальную длину в токенах.",
    ],
    cross_dept=[
        {
            "user_id": "elena.belova",
            "target_org_id": "volga-retail-mortgage",
            "target_dept": "Ипотека",
            "topics": "конверсионные воронки онлайн-заявок, скоринговые модели и тарифная сетка",
            "priority": 2,
        },
        {
            "user_id": "boris.kuznetsov",
            "target_org_id": "nevsky-invest-algo",
            "target_dept": "Алготрейдинг",
            "topics": "риск-метрики деривативного портфеля, VaR и стресс-сценарии ЦБ",
            "priority": 3,
        },
        {
            "user_id": "vadim.antonov",
            "target_org_id": "volga-retail-cards",
            "target_dept": "Карты и платежи",
            "topics": "СБП-сверки, диспуты по chargeback и антифрод-правила",
            "priority": 4,
        },
        {
            "user_id": "natalia.smirnova",
            "target_org_id": "volga-corporate-large",
            "target_dept": "Крупный корпбиз",
            "topics": "синдицированное кредитование, ковенанты и структурирование сделок",
            "priority": 3,
        },
    ],
    company_brands={
        "volga":  {"name": "Волга Банк",      "color": "#DC2626", "initial": "В"},
        "nevsky": {"name": "Невский Капитал", "color": "#1E3A8A", "initial": "Н"},
    },
    archetypes=[
        {
            "provider": "anthropic",
            "utility": "useful",
            "turns": [
                ("user", "Мы запускаем IRB-A подход для розничного ипотечного портфеля. Какой минимальный набор данных нужно собрать для валидации PD-модели в ЦБ?"),
                ("ai",   "Минимальный набор для валидации PD-модели по ипотеке (Положение ЦБ 483-П + Указание 4892-У):\n\n1. Исторические данные: минимум 5 лет наблюдений, включая как минимум один полный экономический цикл. Для ипотеки это обычно 7 лет, поскольку дефолты проявляются с лагом.\n\n2. Определение дефолта: просрочка 90+ дней ИЛИ признание unlikely-to-pay (реструктуризация, списание, судебное производство). Определение должно быть единым для модели и для учётной политики.\n\n3. Размер сегмента: не менее 500 дефолтов в калибровочной выборке. Если меньше — используйте low-default portfolio подход с консервативным допущением.\n\n4. Факторы модели: LTV на момент выдачи и текущий, PTI, кредитная история (NBKI/ОКБ скор + внутренняя история), занятость, регион, цель кредита, возраст заёмщика.\n\n5. Документация валидации: out-of-sample и out-of-time тестирование, AUC/Gini ≥ 0.65 для розницы, тесты на стабильность (PSI < 0.1), бэктестинг прогнозных PD против фактических.\n\nПодготовьте досье в формате Положения 483-П, раздел 11. ЦБ обычно просит три итерации доработки, закладывайте 6-9 месяцев до получения разрешения."),
                ("user", "А как выбрать порог отсечения для PD-бакетов — по квантилям или по бизнес-смыслу?"),
                ("ai",   "Оптимальный подход — гибридный, с приоритетом бизнес-смысла:\n\n(1) Минимум 7 рейтинговых классов для performing + 1 для default (требование ЦБ 483-П).\n(2) Границы классов на монотонных интервалах PD: обычно log-шкала, например 0.1% / 0.3% / 0.8% / 2% / 5% / 12% / 30% / default. Это соответствует S&P-подобной шкале и упрощает бенчмарк.\n(3) Проверка наполненности: в каждом классе не менее 5% портфеля и не менее 50 дефолтов за историю. Если нет — укрупняйте соседние.\n(4) Монотонность дефолт-рейта по классам — обязательно. Если класс 5 даёт меньше дефолтов, чем класс 4 → переопределение границ.\n(5) Тест Herfindahl-концентрации: HHI портфеля по классам < 0.3, иначе модель мало дискриминирует.\n\nКвантильный подход даёт ровное наполнение, но ломает межбанковскую сопоставимость и мешает калибровке TTC. Бизнес-шкала лучше для стресс-тестов ЦБ."),
            ],
        },
        {
            "provider": "openai",
            "utility": "useful",
            "turns": [
                ("user", "У нас растут жалобы клиентов на ложные срабатывания антифрода по СБП-переводам. В часы пик блокируется до 4% транзакций. Как снизить False Positive Rate, не уронив детекцию настоящего мошенничества?"),
                ("ai",   "4% FPR по СБП — очень много (норма для зрелых систем 0.3-0.8%). План снижения в три шага:\n\n1. Измерение базовых показателей (1 неделя): выгрузите 30 дней срабатываний с меткой 'фрод / не фрод' по итогам диспутов. Посчитайте FPR, TPR, precision на каждое правило отдельно. Обычно 3-5 правил из ~50 дают 80% ложных срабатываний — по Парето.\n\n2. Рефакторинг топ-нарушителей (2-3 недели):\n   (а) Правила на геолокацию — добавьте 'доверенные маршруты' на уровне клиента (если он уже 10+ раз переводил на этот телефон/счёт без диспутов, снимайте проверку).\n   (б) Правила на сумму — замените пороговые на скользящие персональные (90-й перцентиль трат клиента за 6 мес., а не общий лимит 30 тыс.).\n   (в) Правила на время — корпоративные клиенты работают ночью чаще розницы; сегментируйте.\n\n3. Скоринговая модель поверх правил (4-8 недель): gradient boosting на фичах из правил + клиентский профиль + граф переводов. Правила оставляете как hard-gates для критичных кейсов (новый получатель + сумма > 100 тыс. = ручной холд), остальное — скорит модель с динамическим порогом. Это стандартная архитектура Сбера/Тинькофф.\n\nОжидаемый эффект: FPR с 4% до 0.8%, TPR сохранится или вырастет (правильно настроенная модель обычно бьёт правила)."),
                ("user", "Какие фичи дают наибольший вклад в антифрод-модель для СБП?"),
                ("ai",   "По опыту крупных российских банков, топ-фичи для СБП-антифрода (в порядке feature importance):\n\n1. Граф переводов: (а) возраст связи отправитель↔получатель, (б) число уникальных отправителей у получателя за 24 ч (признак 'дроппера'), (в) кластеризация получателей по общим IP/устройствам.\n\n2. Поведение клиента: (а) отклонение суммы от медианной за 90 дней (log-шкала), (б) отклонение времени суток от типичного окна, (в) интервал с предыдущей транзакции (всплеск активности).\n\n3. Устройство и сессия: (а) новое устройство / ОС за последние 7 дней, (б) изменение IP/геолокации относительно типичных, (в) признаки удалённого доступа (AnyDesk/TeamViewer детект через SDK).\n\n4. Получатель: (а) возраст счёта/карты получателя, (б) репутация телефона (появление в базах скам-звонков), (в) частота получения P2P против исходящих у получателя.\n\n5. Контекст: (а) время суток × день недели, (б) признак 'только что отклонённая' транзакция, (в) количество попыток за 10 мин.\n\nТипично топ-3 категории (граф + поведение) дают ~70% gain. Обновляйте модель ежемесячно — атакеры быстро адаптируются."),
            ],
        },
        {
            "provider": "google",
            "utility": "trivial",
            "turns": [
                ("user", "Какая сейчас ключевая ставка ЦБ?"),
                ("ai",   "На текущий момент ключевая ставка ЦБ РФ — 21% годовых (установлена решением Совета директоров от 25.10.2024). Следующее опорное заседание — в последнюю пятницу месяца, решение в 13:30 МСК. Актуальность лучше перепроверять на cbr.ru."),
            ],
        },
        {
            "provider": "mistral",
            "utility": "non_work",
            "turns": [
                ("user", "где вкусно пообедать рядом с Москва-Сити в обеденный перерыв?"),
                ("ai",   "В пешей доступности от Москва-Сити: (1) 'Сыроварня' в Афимолле — бизнес-ланч 700-900₽, очереди с 13:00. (2) 'Тануки' на -1 этаже Башни Федерация — быстро, сеты 600-800₽. (3) Food Plaza в Афимолле — от поке до пад-тая, оптимально если коллеги не могут договориться. (4) Для встреч с клиентами — 'Карлсон' или 'Sixty' с видом, но бронируйте за день."),
            ],
        },
        {
            "provider": "anthropic",
            "utility": "trivial",
            "turns": [
                ("user", "Коротко: для запуска новой ипотечной программы нужен ли ОВР и внутренний аудит?"),
                ("ai",   "Да, оба. По Положению ЦБ 716-П любой новый продукт проходит оценку операционного риска (ОВР) до запуска: карта процессов, новые точки отказа, KRI. Внутренний аудит проверяет: соответствие 353-ФЗ (потребкредит), 102-ФЗ (ипотека), корректность расчёта ПСК, раскрытия в договоре. Плюс валидация скоринговой модели службой риск-менеджмента. Без этих согласований комплаенс-офицер не подпишет запуск."),
            ],
        },
    ],
)
